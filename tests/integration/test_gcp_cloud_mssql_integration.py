...
def test_mssql_backended_gcp_simulation_model_upload_and_access_via_api(
    test_model, training_params, conda_env
):
    with DockerCompose(
        context=".",
        compose_file_name=["docker-compose.gcp-mssql-test.yaml"],
        # pull=True,
        build=True,
    ) as compose:
        ...
        experiment_name = "gcp-cloud-mssql-experiment"
        model_name = "test-gcp-mssql-model"
        stage_name = "Staging"
